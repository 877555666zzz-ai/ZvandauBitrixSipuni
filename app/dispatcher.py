...
async def process_new_lead(
    ...
    if not skip_new_lead_checks and await _phone_recently_handled(client_phone, lead_id):
        ...
        # Не молчим: если это ошибочное совпадение (разные клиенты, случайно
        # одинаковый номер) — оператор должен это увидеть в сделке, а не
        # узнать, что лид "потерялся", только когда клиент пожалуется.
        await add_lead_comment(
            lead_id,
            "Автодозвон: не запущен — на этот номер уже звонили или звонок "
            "уже идёт по другой сделке за последние 24 часа. Если это разные "
            "клиенты — обработайте вручную.",
        )
