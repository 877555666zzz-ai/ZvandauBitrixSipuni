...
class Settings(BaseSettings):
    ...
    # «Передышка» оператора после звонка: короткий гарантированный зазор перед
    # новым звонком (даже если оператор ничего не нажал), чтобы не захлёбывался.
    # Основное управление — ручная кнопка «Пауза/Возобновить» на странице.
    MANAGER_BREATHER_SECONDS: int = 60
