...
async def reset_dashboard_stats(_: None = Depends(require_auth)):
    ...
    приоритет) и снимает занятость/паузу/«на звонке». САМИХ менеджеров, их
    логины/пароли и отделы НЕ трогает. Необратимо — подтверждение только
    на фронтенде (кнопка + confirm).
    """
    async with async_session_maker() as session:
        logs = (await session.execute(
        ...
        # Обнуляем счётчики менеджеров, оставляя их самих, логины и отделы.
        managers = (await session.execute(select(Manager))).scalars().all()
        for m in managers:
            m.accepted_calls = 0
            m.missed = 0
            m.priority_score = 0.5
            m.priority_total = 0.0
            m.priority_ok = 0.0
            m.priority_updated_at = None
            m.busy_until = None
            m.on_call = False
