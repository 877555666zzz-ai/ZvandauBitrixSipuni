...
- ✅ **Отделы (многопоточность по проектам)** — изоляция «Яндекс 360» /
  «Яндекс Такси Казахстан» и любых будущих проектов: каждый отдел жёстко
  привязан к своей воронке Битрикс24, менеджеры работают только в своей
  «песочнице» (см. раздел [Отделы](#отделы-многопоточность-по-проектам))

---

## Отделы (многопоточность по проектам)

Реализация ТЗ «Модернизация системы автодозвона (Многопоточность по отделам)».

### Как это работает

1. Через `/admin/departments` (или напрямую через API) создаётся **отдел**:
   название + `CATEGORY_ID` воронки Битрикс24 + стадия-триггер («Тёплые») +
   опционально стадии НДЗ / НДЗ 2.
2. Каждому **менеджеру** назначается отдел (там же, на странице, или через
   `PUT /managers/{id}` с `department_id`).
3. Когда в Битрикс24 сделка попадает в стадию-триггер нужной воронки,
   `/bitrix/webhook/deal` находит подходящий отдел по паре
   (`CATEGORY_ID`, `STAGE_ID`) и запускает автодозвон **только** среди
   менеджеров этого отдела.
4. Недозвон/повтор/финальная неудача двигают сделку по стадиям НДЗ / НДЗ 2
   **этого конкретного отдела** (а не общим настройкам из `.env`).
5. Ручной перевод звонка (кнопка «Перевести») тоже ограничен коллегами из
   того же отдела — лид не может «утечь» в чужой проект даже вручную.

### Обратная совместимость

Если ни один отдел ещё не создан, всё продолжает работать по старому —
через `BITRIX_DEAL_CATEGORY_ID` / `BITRIX_STAGE_TRIGGER` /
`BITRIX_STAGE_NDZ` / `BITRIX_STAGE_NDZ2` в `.env`, менеджеры без отдела
участвуют в общем пуле. Как только вы создаёте первый отдел, `/bitrix/webhook/deal`
переключается на строгий режим: сделка обрабатывается, только если её
воронка+стадия совпали с каким-то отделом, иначе — игнорируется (это и есть
изоляция «Исключить пересечения»).

### Админка отделов

Страница `/admin/departments` (требует того же HTTP Basic, что и dashboard):

- таблица отделов с редактированием прямо в строке (название, воронка,
  стадии) и удалением;
- форма добавления нового отдела;
- таблица менеджеров с выпадающим списком «Отдел» — сохраняется сразу.

### API

| Метод | URL | Описание |
|---|---|---|
| GET | `/departments` | Список отделов (+ кол-во менеджеров) |
| POST | `/departments` | Создать отдел |
| PUT | `/departments/{id}` | Изменить отдел (в т.ч. «переехать» на другую воронку) |
| DELETE | `/departments/{id}` | Удалить отдел (менеджеры отвязываются, не удаляются) |
| GET | `/admin/departments` | HTML-страница управления отделами |

`POST /managers` и `PUT /managers/{id}` теперь принимают `department_id`
(и `PUT` дополнительно — `clear_department: true`, чтобы снять привязку).

### Как узнать `CATEGORY_ID` и `STAGE_ID` нужной воронки

```
https://<portal>.bitrix24.ru/rest/1/<TOKEN>/crm.category.list.json?entityTypeId=2
https://<portal>.bitrix24.ru/rest/1/<TOKEN>/crm.status.list.json?filter[ENTITY_ID]=DEAL_STAGE_<CATEGORY_ID>
```

`STAGE_ID` в ответе — это и есть значение для поля «Стадия-триггер» отдела
(например `C12:NEW` или `UC_MLQD5A`).
